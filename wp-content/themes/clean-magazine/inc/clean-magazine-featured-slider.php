...
if( !function_exists( 'clean_magazine_featured_slider' ) ) :
/**
 * Add slider.
 *
 ...
 * @since Clean Magazine 0.2
 */
function clean_magazine_featured_slider() {
	...
	if ( $enableslider == 'entire-site' || ( ( is_front_page() || ( is_home() && $page_for_posts != $page_id ) ) && $enableslider == 'homepage' ) ) {
		if( ( !$clean_magazine_featured_slider = get_transient( 'clean_magazine_featured_slider' ) ) ) {
			echo '<!-- refreshing cache -->';

			$clean_magazine_featured_slider = '
				<section id="feature-slider">
					<div class="wrapper">
						<div class="cycle-slideshow"
							...
							// Select Slider
							if ( $sliderselect == 'demo-featured-slider' && function_exists( 'clean_magazine_demo_slider' ) ) {
								$clean_magazine_featured_slider .=  clean_magazine_demo_slider( $options );
							}
							elseif ( $sliderselect == 'featured-page-slider' && function_exists( 'clean_magazine_page_slider' ) ) {
								$clean_magazine_featured_slider .=  clean_magazine_page_slider( $options );
							}
			...
			set_transient( 'clean_magazine_featured_slider', $clean_magazine_featured_slider, 86940 );
		}
		echo $clean_magazine_featured_slider;
	}
}
endif;
add_action( 'clean_magazine_before_content', 'clean_magazine_featured_slider', 10 );


if ( ! function_exists( 'clean_magazine_demo_slider' ) ) :
/**
 ...
 *
 */
function clean_magazine_demo_slider( $options ) {
	$clean_magazine_demo_slider ='
	...
	return $clean_magazine_demo_slider;
}
endif; // clean_magazine_demo_slider


if ( ! function_exists( 'clean_magazine_page_slider' ) ) :
/**
...
function clean_magazine_page_slider( $options ) {
		...
		while ( $get_featured_posts->have_posts()) : $get_featured_posts->the_post(); $i++;
			...
			<article class="'.$classes.'">
				...
				if ( has_post_thumbnail() ) {
					$clean_magazine_page_slider .= '<a title="Permalink to '.the_title('','',false).'" href="' . get_permalink() . '">
						'. get_the_post_thumbnail( $post->ID, 'clean-magazine-slider', array( 'title' => esc_attr( $title_attribute ), 'alt' => esc_attr( $title_attribute ), 'class'	=> 'attached-page-image' ) ).'
					</a>';
				}
				else {
					//Default value if there is no first image
					$clean_magazine_image = '<img class="pngfix wp-post-image" src="'.get_template_directory_uri().'/images/gallery/no-featured-image-1920x800.jpg" >';

					//Get the first image in page, returns false if there is no image
					$clean_magazine_first_image = clean_magazine_get_first_image( $post->ID, 'clean-magazine-slider', array( 'title' => esc_attr( $title_attribute ), 'alt' => esc_attr( $title_attribute ), 'class' => 'attached-page-image' ) );

					//Set value of image as first image if there is an image present in the page
					if ( '' != $clean_magazine_first_image ) {
						$clean_magazine_image =	$clean_magazine_first_image;
					}

					$clean_magazine_page_slider .= '<a title="Permalink to '.the_title('','',false).'" href="' . get_permalink() . '">
						'. $clean_magazine_image .'
					</a>';
				}
				...
				<div class="entry-container">
					<header class="entry-header">
						...
						<div class="assistive-text">'.clean_magazine_page_post_meta().'</div>
					</header>';
					if( $excerpt !='') {
						$clean_magazine_page_slider .= '<div class="entry-summary"><p>'. $excerpt.'</p></div>';
					}
					$clean_magazine_page_slider .= '
				</div><!-- .entry-container -->
			</article><!-- .slides -->';
	...
	return $clean_magazine_page_slider;
}
endif; // clean_magazine_page_slider
